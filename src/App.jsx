...
const voteModule = sdk.getVoteModule("0xF44Cc4ce154EdF6CE4Eb3C43892f11CbdbF13554");
...
const App = () => {
  ...
  // Also need to check if the user already voted
  useEffect(() => {
    if (!hasClaimedNFT) {
      return;
    }

    // If we haven't finished pulling the proposals in the useEffect above we can't check if the user voted yet
    if (!proposals.length) {
      return;
    }
    ...
    voteModule
      .hasVoted(proposals[0].proposalId, address) // Only 1 proposal at a time?
      .then((hasVoted) => {
        setHasVoted(hasVoted);
        console.log("User has already voted");
      })
      .catch((err) => {
        console.error("Failed to check if walled has voted", err);
      });
